...
vec4 ntsc(sampler2D src){
	float frameTime = float(frame)/60.0;
	vec2 uv = gl_FragCoord.xy / resolution.xy;

	return vec4(texture2D(src, uv).rgb, 1.0);

	
	float u_vhs  = pow(rand(frameTime*0.013), 20.0);
	float u_sync = 0.5*rand(frameTime*0.05);
	...
	for (int xint = 10; xint >= 0; xint -= 1){
		...
		uv1.x += vn;
		uv2.x -= vn;
		yuv += rgbtoyuv * (texture2D(tex0, uv1).rgb) * mult;
		yuv += rgbtoyuv * (texture2D(tex0, uv2).rgb) * mult;
		yuv.r += 2.5*vn;
	}
	...
	if (u_vhs > 0.0){
		...
		na = (0.5 + sin(uv.y * 3.0 - frameTime * 6.0) * 0.5) * noiseenable;n
		yuv.g = mix(yuv.g, rand( vec2( frameTime * 9.0 + uv_n.x * 13.0 + uv_n.y * 12.0)), pow(na, 5.5));
	}
	
	return vec4(yuvtorgb * yuv * shade, 1.0)*0.02;
